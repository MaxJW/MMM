...
var defaults={descriptionTarget:null,maxTemperatureTarget:null,minTemperatureTarget:null,windSpeedTarget:null,humidityTarget:null,sunriseTarget:null,sunsetTarget:null,placeTarget:null,iconTarget:null,units:'c',city:null,lat:null,lng:null,key:null,lang:'en',success:function(){},error:function(message){}}
var plugin=this;var el=$(this);var apiURL;plugin.settings={}
plugin.settings=$.extend({},defaults,options);var s=plugin.settings;apiURL='https://api.openweathermap.org/data/2.5/weather?lang='+s.lang;apiURL+='&q='+s.city.replace(' ','');apiURL+='&appid='+s.key;var formatTime=function(unixTimestamp){var milliseconds=unixTimestamp*1000;var date=new Date(milliseconds);var hours=date.getHours();if(hours>12){hoursRemaining=24-hours;hours=12-hoursRemaining}
...
$(s.descriptionTarget).text(data.weather[0].description);if(s.iconTarget!=null&&data.weather[0].id!=null){var iconID=data.weather[0].id;var iconURL='wi wi-owm-'+iconID;$(s.iconTarget).attr('class',iconURL)}
if(s.placeTarget!=null){$(s.placeTarget).text(data.name+', '+data.sys.country)}
if(s.windSpeedTarget!=null){var windSpeedNum=Math.round(data.wind.speed)+'';var windSpeedIcon='wi wi-wind-beaufort-'+windSpeedNum;$(s.windSpeedTarget).attr('class',windSpeedIcon)}
